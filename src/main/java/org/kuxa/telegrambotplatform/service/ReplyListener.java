...
import org.kuxa.telegrambotplatform.bots.GatewayBot;
...
public class ReplyListener {
    ...
    private final GatewayBot gatewayBot;
    ...
    public void onReply(BotResponse response) {
        SendMessage message = SendMessage.builder()
                ...
                .build();
        try {
            gatewayBot.getTelegramClient().execute(message);
        } catch (TelegramApiException e) {
            log.error("Failed to send reply to chatId={}: {}", response.getChatId(), e.getMessage(), e);
        }
    }
}
